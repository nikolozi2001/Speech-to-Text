...
export default function SettingsModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const [outputFormat, setOutputFormat] = useState('ტექსტი');
  const [punctuation, setPunctuation] = useState('ავტომატური');
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={styles.sheet}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <SelectField
            label="ენა/ლოკალი"
            options={['ქართული (საქართველო)', 'English (US)', 'Русский', 'Deutsch', 'Français']}
            value={language}
            onChange={setLanguage}
          />

          <SelectField
            label="ინტელექტი (გეგმები)"
            options={['სტანდარტული', 'გაფართოებული', 'პრემიუმი']}
            value={model}
            onChange={setModel}
          />

          <SelectField
            label="STT პროვაიდერი"
          ...
          <SelectField
            label="გამოტანის ფორმატი"
            options={['ტექსტი', 'JSON', 'SRT', 'VTT']}
            value={outputFormat}
            onChange={setOutputFormat}
          />

          <View style={styles.fieldWrapper}>
            <Text style={styles.fieldLabel}>პუნქტუაცია</Text>
            <View style={styles.radioRow}>
              {['ავტომატური', 'გამორთული'].map((opt) => (
                <TouchableOpacity
                  key={opt}
